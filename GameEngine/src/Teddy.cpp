...
void Teddy::ReadAnimationData(const char * filename)
{
	FILE* file;
...
void Teddy::SetAnimationPose( )
{
	
	// First thing, get the first frame of animation
	animator.ProcessAnimation( tCurrent,pHead,20 );
...
void Teddy::walk_anim_node(GameObject *node )
{
	setBonePose( node );
...
MyGameObjectAnim* Teddy::findByIndex(std::vector<MyGameObjectAnim*> list, int index)
{
	//GameObjectAnim *a = 0;
...
Frame_Bucket *  Teddy::getHeadFrameBucket()
{
	
	return pHead;
}

void Teddy::findMaxTime(Time &tMax)
{
	animator.findMaxTime(tMax,pHead);

}

void Teddy::ProcessAnimation(Time tCurr)
{


	animator.ProcessAnimation(tCurr,pHead,20);


}

void Teddy::ChangeAnimation()
{
